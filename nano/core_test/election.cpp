...
TEST (election, quorum_minimum_update_weight_before_quorum_checks)
{
	...
	auto amount = ((nano::uint256_t (node_config.online_weight_minimum.number ()) * node1.online_reps.online_weight_quorum) / 100).convert_to<nano::uint128_t> () - 1;
	nano::keypair key1;
	nano::block_builder builder;
	auto send1 = builder.state ()
	             .account (nano::dev_genesis_key.pub)
	...
	auto vote1 (std::make_shared<nano::vote> (nano::dev_genesis_key.pub, nano::dev_genesis_key.prv, 1, send1));
	ASSERT_EQ (nano::vote_code::vote, node1.active.vote (vote1, true));
	auto vote2 (std::make_shared<nano::vote> (key1.pub, key1.prv, 2, send1));
